...
export function ElectricityAreaChart() {
  ...
  const { values, loading: isLoading } = useEquipmentValues(
  ...
  useEffect(() => {
    if (values.length > 0) {
      const electricValues = values.filter((v) =>
        v.equipmentName.toLowerCase().includes("electric")
      );

      const current =
        electricValues.find((v) => v.valueName === "electricCurrent")
          ?.valueResponse || 0;
      const power =
        electricValues.find((v) => v.valueName === "active-power")
          ?.valueResponse || 0;
      const voltage =
        electricValues.find((v) => v.valueName === "voltage")?.valueResponse ||
        0;
      ...
      setChartData((prev) => {
        const newData = [
          ...prev,
          { timestamp: timeStr, current, power, voltage },
        ];
