    ...
    <SidebarProvider>
      ...
      <SidebarInset className="bg-gradient-to-br from-slate-50 to-slate-100 dark:from-blue-950 dark:to-slate-900 flex flex-col h-screen overflow-hidden">
        <header
          className={`flex ${
            isMobile ? "h-14" : "h-16"
          } shrink-0 items-center gap-2 border-b border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm transition-all ease-in-out`}
        >
          <div className="flex items-center gap-3 px-4 md:px-6">
            <SidebarTrigger className="-ml-1 w-5 h-5 text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-100 transition-colors" />
            <Separator orientation="vertical" className="h-5 bg-slate-200 dark:bg-slate-700" />
            <h1 className={`${isMobile ? "text-lg" : "text-xl"} font-bold bg-gradient-to-r from-blue-600 to-blue-800 dark:from-blue-400 dark:to-blue-600 bg-clip-text text-transparent`}>
              {selectedFacility}
            </h1>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-4">
          <div className="bg-gradient-to-br from-white to-slate-50 dark:from-slate-800 dark:to-slate-900 p-5 rounded-2xl shadow-lg shadow-slate-200/50 dark:shadow-none border border-slate-200/50 dark:border-slate-700/50 hover:shadow-xl hover:scale-[1.02] transition-all">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h3 className="text-sm font-medium text-slate-600 dark:text-slate-300">Active Rooms</h3>
            </div>
            <div className="space-y-1">
              <span className="text-2xl font-bold text-slate-900 dark:text-white">
                {stats.rooms.active}/{stats.rooms.total}
              </span>
              <p className="text-sm text-slate-500 dark:text-slate-400">Currently in use</p>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-white to-slate-50 dark:from-slate-800 dark:to-slate-900 p-5 rounded-2xl shadow-lg shadow-slate-200/50 dark:shadow-none border border-slate-200/50 dark:border-slate-700/50 hover:shadow-xl hover:scale-[1.02] transition-all">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h3 className="text-sm font-medium text-slate-600 dark:text-slate-300">Highest Electricity Usage</h3>
            </div>
            <div className="space-y-1">
              <span className="text-2xl font-bold text-slate-900 dark:text-white">
                {stats.electricity.value}
              </span>
              <p className="text-sm text-slate-500 dark:text-slate-400">{stats.electricity.highestFloor}</p>
            </div>
          </div>
            ...
            <div className="flex items-center gap-3 mb-3">
              <div className="p-2 rounded-lg bg-red-100 dark:bg-red-900/50">
                <svg className="w-4 h-4 text-red-600 dark:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h8M8 14h8M8 18h8M8 6h8M5 6h.01M5 10h.01M5 14h.01M5 18h.01M21 12c0 4.418-3.582 8-8 8H9.414l-4.707 4.707A1 1 0 014 22V2a1 1 0 011.707-.707L9.414 6H13c4.418 0 8 3.582 8 8z" />
                </svg>
              </div>
              <h3 className="text-sm font-medium text-slate-600 dark:text-slate-300">Highest CO2 Emissions</h3>
            </div>
            <div className="space-y-1">
              <span className="text-2xl font-bold text-slate-900 dark:text-white">
                {stats.co2.value}
              </span>
              <p className="text-sm text-slate-500 dark:text-slate-400">{stats.co2.highestFloor}</p>
            </div>
          </div>
            ...
            <div className="flex items-center gap-3 mb-3">
              <div className="p-2 rounded-lg bg-sky-100 dark:bg-sky-900/50">
                <svg className="w-4 h-4 text-sky-600 dark:text-sky-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M3 14h18M3 18h18M3 6h18M5 6h.01M5 10h.01M5 14h.01M5 18h.01M21 12c0 4.418-3.582 8-8 8H9.414l-4.707 4.707A1 1 0 014 22V2a1 1 0 011.707-.707L9.414 6H13c4.418 0 8 3.582 8 8z" />
                </svg>
              </div>
              <h3 className="text-sm font-medium text-slate-600 dark:text-slate-300">Highest Water Usage</h3>
            </div>
            <div className="space-y-1">
              <span className="text-2xl font-bold text-slate-900 dark:text-white">
                {stats.water.value}
              </span>
              <p className="text-sm text-slate-500 dark:text-slate-400">{stats.water.highestFloor}</p>
            </div>
          </div>
        </div>
